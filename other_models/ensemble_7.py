...
train = pd.DataFrame()
pc_list = []
train_e1 = pd.read_csv("msr_train.csv")
train_e2 = pd.read_csv("msr_test.csv")
train_e3 = pd.read_csv("tpc.csv")
train_m1 = pd.read_csv("train.csv")
pc_list.append(train_e1)
pc_list.append(train_e2)
pc_list.append(train_e3)
pc_list.append(train_m1)
#test = pd.read_csv("../input/test.csv")[:100]
train = pd.concat(pc_list)
...
print("Num of train null vals ", train.isnull().sum())
print("Num of test null vals ", test.isnull().sum())
...
# In[12]:
print("Num null valus of train and test after filling na with empty")

print("Train null vals num ", train.isnull().sum())
print("Test null vals num ", test.isnull().sum())
...
print("Preview of some train and test Qs ")
for i in range(6):
...
def process_questions(question_list, questions, question_list_name, dataframe):
    ...
    print("For ", question_list_name)
    print("num of Qs = ", len(questions))
    print("dataframe len =", len(dataframe))
    for question in questions:
        question_list.append(text_to_wordlist(question))
        if len(question_list) % 100000 == 0:
            print("Q list len = ", len(question_list))
            print("Dataframe len = ", len(dataframe))
...
model5.add(LSTM(embedding_dim,return_sequences=True))
...
model6.add(LSTM(embedding_dim,return_sequences=True))
...
modelc.add(LSTM(embedding_dim))
modelc.add(BatchNormalization())
modelc.add(Dropout(dropout))
modelc.add(Dense(units*2, kernel_initializer=weights, bias_initializer=bias))
modelc.add(BatchNormalization())
modelc.add(Activation('relu'))
modelc.add(Dropout(dropout))

modelc.add(Dense(units, kernel_initializer=weights, bias_initializer=bias))
modelc.add(BatchNormalization())
modelc.add(Activation('relu'))
modelc.add(Dropout(dropout))
...
model.add(Dense(units*4, kernel_initializer=weights, bias_initializer=bias))
...
val_q1 = train_q1[-40000:]
val_q2 = train_q2[-40000:]
val_l = y_train[-40000:]

train_q1 = train_q1[:-40000]
train_q2 = train_q2[:-40000]
train_l = y_train[:-40000]

weight_val = np.ones(len(val_l))
weight_val *= 0.472001959
weight_val[val_l==0] = 1.309028344
# In[178]:
...
history = model.fit([train_q1, train_q2, train_q1, train_q2, train_q1, train_q2],
                    train_l,
                    batch_size=256,
                    epochs=70, #Use 100, I reduce it for Kaggle,
                    validation_data = ([val_q1,val_q2,val_q1,val_q2,val_q1,val_q2],val_l, weight_val),
                    verbose=True,
                    shuffle=True,
...
predictions += model.predict([test_q2, test_q1, test_q2, test_q1, test_q2, test_q1], verbose= True)
predictions /= 2
